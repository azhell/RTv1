...
int8_t	ft_shadows(t_rtv1 *rt, t_shadow *sh)
{
	// t_figure	*fig;
	// t_ray		ray;
...
// 	color_i = 0.3;
// 	tmp = rt->figure;
// 	vec = ft_vec_normalize(light->pos - ray->light->point);
// 	vec_light = ft_vec_normalize(ray->light->point - light->pos);
...
// 	len_N = ft_vec_len(ray->light->point, light->pos);
// 	len_R = ft_vec_len(rt->camera.pos, ray->light->point);
// 	// fix color after
...
// 		//printf("%f | ", color_i);
// 		if (!res && angle_lht > 0)
// 			color_i += rt->light->intense * angle_lht / (len_N * len_R / 500);
// 		if (color_i > 1.0)
// 			color_i = 1.0;
// 		if (color_i < 0.0)
// 			color_i = 0;
